...
def _message_text(content: Optional[Union[str, List[Dict[str, Any]]]]) -> str:
...
def _user_signal_text(text: str) -> str:
...
def _absolute_path_hint(path_hint: Optional[str]) -> Optional[str]:
    if not path_hint:
        return None
...
def _looks_like_file_path(path_hint: Optional[str]) -> bool:
    if not path_hint:
        return False
...
def _looks_like_directory_request(text: str) -> bool:
    lowered = text.lower()
...
def _build_path_arguments(tool: Dict[str, Any], path_hint: Optional[str], candidates: List[str], fallback: str) -> Optional[Dict[str, Any]]:
    absolute = _absolute_path_hint(path_hint)
    if not absolute:
        return None
    field = _preferred_tool_field(tool, candidates, fallback)
    return {field: absolute}


def _can_call_read_file(tool: Dict[str, Any]) -> bool:
    required = set(_tool_required(tool))
    return not any(field in required for field in ["startLine", "start_line", "endLine", "end_line", "line"])


def _can_call_list_dir(path_hint: Optional[str], user_text: str) -> bool:
    return bool(path_hint) and not _looks_like_file_path(path_hint) and _looks_like_directory_request(user_text)


def _can_call_read_path(path_hint: Optional[str], user_text: str) -> bool:
    return bool(path_hint) and _looks_like_file_read_request(user_text)


def _build_search_arguments(tool: Dict[str, Any], user_text: str, path_hint: Optional[str]) -> Dict[str, Any]:
    ...
    pattern_field = _preferred_tool_field(tool, ["pattern", "query", "text"], "pattern")
    arguments: Dict[str, Any] = {pattern_field: pattern}
    absolute = _absolute_path_hint(path_hint)
    if absolute:
        path_field = _preferred_tool_field(tool, ["filePath", "dirPath", "path"], "path")
        arguments[path_field] = absolute
    return arguments


def _build_grep_arguments(tool: Dict[str, Any], user_text: str) -> Dict[str, Any]:
    pattern_match = re.search(r"(?:search|find|grep|tìm)\s+(?:for\s+)?[`'\"]?([^`'\"\n]+)[`'\"]?", user_text, re.IGNORECASE)
    pattern = pattern_match.group(1).strip() if pattern_match else user_text.strip()
    query_field = _preferred_tool_field(tool, ["query", "pattern", "text"], "query")
    regexp_field = _preferred_tool_field(tool, ["isRegexp", "is_regex"], "isRegexp")
    return {query_field: pattern, regexp_field: False}


def _build_pwd_arguments(tool: Dict[str, Any]) -> Dict[str, Any]:
    command_field = _preferred_tool_field(tool, ["command", "cmd"], "command")
    return {command_field: "pwd"}


def _build_read_file_arguments(tool: Dict[str, Any], path_hint: Optional[str]) -> Optional[Dict[str, Any]]:
    return _build_path_arguments(tool, path_hint, ["filePath", "path"], "path")


def _build_list_dir_arguments(tool: Dict[str, Any], path_hint: Optional[str]) -> Optional[Dict[str, Any]]:
    return _build_path_arguments(tool, path_hint, ["dirPath", "filePath", "path"], "path")


def _tool_call_with_arguments(name: str, arguments: Optional[Dict[str, Any]], request_id: str) -> Optional[Dict[str, Any]]:
    if not arguments:
        _log_outgoing_tool_call(request_id, None)
        return None
    tool_call = _build_tool_call(name, arguments)
    _log_outgoing_tool_call(request_id, tool_call)
    return tool_call
...
def _looks_like_pwd_request(text: str) -> bool:
    lowered = text.lower()
    return any(word in lowered for word in ["pwd", "working directory", "workdir", "current directory"])


def _looks_like_inline_code_blob(text: str) -> bool:
    lowered = text.lower()
    return "...existing code..." in lowered or len(text) > 500


def _should_avoid_tool_call(user_text: str, path_hint: Optional[str]) -> bool:
    return _looks_like_inline_code_blob(user_text) and not path_hint


def _log_no_tool_decision(request_id: str, reason: str) -> None:
    logger.info(f"[CHAT] [req_{request_id}] no_tool_call_reason={reason}")


def _maybe_tool_call(name: str, arguments: Optional[Dict[str, Any]], request_id: str) -> Optional[Dict[str, Any]]:
    return _tool_call_with_arguments(name, arguments, request_id)


def _resolve_tool_path(path_hint: Optional[str]) -> Optional[str]:
    return _absolute_path_hint(path_hint)


def _normalized_path_hint(path_hint: Optional[str]) -> Optional[str]:
    return _resolve_tool_path(path_hint)


def _is_file_request_without_range(tool: Dict[str, Any]) -> bool:
    return _can_call_read_file(tool)


def _safe_path_hint(path_hint: Optional[str]) -> Optional[str]:
    return _normalized_path_hint(path_hint)


def _safe_path_args(tool: Dict[str, Any], path_hint: Optional[str], candidates: List[str], fallback: str) -> Optional[Dict[str, Any]]:
    return _build_path_arguments(tool, _safe_path_hint(path_hint), candidates, fallback)


def _safe_search_args(tool: Dict[str, Any], user_text: str, path_hint: Optional[str]) -> Dict[str, Any]:
    return _build_search_arguments(tool, user_text, _safe_path_hint(path_hint))


def _path_hint_for_tool(path_hint: Optional[str]) -> Optional[str]:
    return _safe_path_hint(path_hint)


def _tool_path_is_file(path_hint: Optional[str]) -> bool:
    return _looks_like_file_path(path_hint)


def _tool_path_is_dir_request(path_hint: Optional[str], user_text: str) -> bool:
    return _can_call_list_dir(path_hint, user_text)


def _tool_path_is_read_request(path_hint: Optional[str], user_text: str) -> bool:
    return _can_call_read_path(path_hint, user_text)


def _tool_call_args_or_none(arguments: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    return arguments


def _search_request(text: str) -> bool:
    return _looks_like_search_request(text)


def _pwd_request(text: str) -> bool:
    return _looks_like_pwd_request(text)


def _avoid_tool_call(text: str, path_hint: Optional[str]) -> bool:
    return _should_avoid_tool_call(text, path_hint)


def _log_skip_tool(request_id: str, reason: str) -> None:
    _log_no_tool_decision(request_id, reason)


def _tool_read_without_range(tool: Dict[str, Any]) -> bool:
    return _is_file_request_without_range(tool)


def _tool_dir_args(tool: Dict[str, Any], path_hint: Optional[str]) -> Optional[Dict[str, Any]]:
    return _safe_path_args(tool, path_hint, ["dirPath", "filePath", "path"], "path")


def _tool_file_args(tool: Dict[str, Any], path_hint: Optional[str]) -> Optional[Dict[str, Any]]:
    return _safe_path_args(tool, path_hint, ["filePath", "path"], "path")


def _tool_search_args(tool: Dict[str, Any], user_text: str, path_hint: Optional[str]) -> Dict[str, Any]:
    return _safe_search_args(tool, user_text, path_hint)


def _tool_pwd_args(tool: Dict[str, Any]) -> Dict[str, Any]:
    return _build_pwd_arguments(tool)


def _maybe_return_tool(name: str, args: Optional[Dict[str, Any]], request_id: str) -> Optional[Dict[str, Any]]:
    return _maybe_tool_call(name, _tool_call_args_or_none(args), request_id)


def _user_requests_list_dir(path_hint: Optional[str], user_text: str) -> bool:
    return _tool_path_is_dir_request(path_hint, user_text)


def _user_requests_read_file(path_hint: Optional[str], user_text: str) -> bool:
    return _tool_path_is_read_request(path_hint, user_text)


def _user_requests_search(user_text: str) -> bool:
    return _search_request(user_text)


def _user_requests_pwd(user_text: str) -> bool:
    return _pwd_request(user_text)


def _skip_due_to_inline_blob(user_text: str, path_hint: Optional[str]) -> bool:
    return _avoid_tool_call(user_text, path_hint)


def _skip_tool(request_id: str, reason: str) -> None:
    _log_skip_tool(request_id, reason)


def _file_like_path(path_hint: Optional[str]) -> bool:
    return _tool_path_is_file(path_hint)


def _absolute_hint(path_hint: Optional[str]) -> Optional[str]:
    return _safe_path_hint(path_hint)


def _tool_search_with_path(tool: Dict[str, Any], user_text: str, path_hint: Optional[str]) -> Dict[str, Any]:
    return _tool_search_args(tool, user_text, path_hint)


def _tool_dir_with_path(tool: Dict[str, Any], path_hint: Optional[str]) -> Optional[Dict[str, Any]]:
    return _tool_dir_args(tool, path_hint)


def _tool_file_with_path(tool: Dict[str, Any], path_hint: Optional[str]) -> Optional[Dict[str, Any]]:
    return _tool_file_args(tool, path_hint)


def _tool_command_args(tool: Dict[str, Any]) -> Dict[str, Any]:
    return _tool_pwd_args(tool)


def _return_tool(name: str, args: Optional[Dict[str, Any]], request_id: str) -> Optional[Dict[str, Any]]:
    return _maybe_return_tool(name, args, request_id)


def _should_skip_tool(user_text: str, path_hint: Optional[str]) -> bool:
    return _skip_due_to_inline_blob(user_text, path_hint)


def _directory_request(user_text: str, path_hint: Optional[str]) -> bool:
    return _user_requests_list_dir(path_hint, user_text)


def _file_request(user_text: str, path_hint: Optional[str]) -> bool:
    return _user_requests_read_file(path_hint, user_text)


def _search_request_needed(user_text: str) -> bool:
    return _user_requests_search(user_text)


def _pwd_request_needed(user_text: str) -> bool:
    return _user_requests_pwd(user_text)


def _skip_reason_log(request_id: str, reason: str) -> None:
    _skip_tool(request_id, reason)


def _absolute_or_none(path_hint: Optional[str]) -> Optional[str]:
    return _absolute_hint(path_hint)


def _use_read_file(tool: Dict[str, Any]) -> bool:
    return _tool_read_without_range(tool)


def _dir_args(tool: Dict[str, Any], path_hint: Optional[str]) -> Optional[Dict[str, Any]]:
    return _tool_dir_with_path(tool, path_hint)


def _file_args(tool: Dict[str, Any], path_hint: Optional[str]) -> Optional[Dict[str, Any]]:
    return _tool_file_with_path(tool, path_hint)


def _search_args(tool: Dict[str, Any], user_text: str, path_hint: Optional[str]) -> Dict[str, Any]:
    return _tool_search_with_path(tool, user_text, path_hint)


def _command_args(tool: Dict[str, Any]) -> Dict[str, Any]:
    return _tool_command_args(tool)


def _emit_tool(name: str, args: Optional[Dict[str, Any]], request_id: str) -> Optional[Dict[str, Any]]:
    return _return_tool(name, args, request_id)


def _should_skip(user_text: str, path_hint: Optional[str]) -> bool:
    return _should_skip_tool(user_text, path_hint)


def _log_skip(request_id: str, reason: str) -> None:
    _skip_reason_log(request_id, reason)


def _path_is_file(path_hint: Optional[str]) -> bool:
    return _file_like_path(path_hint)


def _path_abs(path_hint: Optional[str]) -> Optional[str]:
    return _absolute_or_none(path_hint)


def _range_fallback_allowed(user_text: str) -> bool:
    return _looks_like_edit_request(user_text)


def _range_fallback(tool: Dict[str, Any], path_hint: Optional[str], user_text: str) -> Optional[Dict[str, Any]]:
    return _fallback_tool_call_from_schema(tool, _path_abs(path_hint), user_text)


def _range_fallback_any(available: Dict[str, Dict[str, Any]], path_hint: Optional[str], user_text: str) -> Optional[Dict[str, Any]]:
    if not _range_fallback_allowed(user_text):
        return None
    for tool in available.values():
        fallback = _range_fallback(tool, path_hint, user_text)
        if fallback is not None:
            return fallback
    return None


def _first_schema_compatible_tool_call(available: Dict[str, Dict[str, Any]], path_hint: Optional[str], user_text: str) -> Optional[Dict[str, Any]]:
    return _range_fallback_any(available, path_hint, user_text)


def _infer_openai_tool_call(req: ChatRequest, request_id: str = "") -> Optional[Dict[str, Any]]:
    if not req.tools:
        return None
    ...
    signal_text = _user_signal_text(user_text)
    lowered = signal_text.lower()
    path_hint = _path_abs(_extract_path_hint(signal_text))

    if _should_skip(signal_text, path_hint):
        _log_skip(request_id, "inline_code_blob")
        _log_outgoing_tool_call(request_id, None)
        return None

    if path_hint and _file_request(signal_text, path_hint):
        if "read_file" in available and _use_read_file(available["read_file"]):
            return _emit_tool("read_file", _file_args(available["read_file"], path_hint), request_id)

    if path_hint and _directory_request(signal_text, path_hint):
        if "list_dir" in available and not _path_is_file(path_hint):
            return _emit_tool("list_dir", _dir_args(available["list_dir"], path_hint), request_id)
        _log_skip(request_id, "list_dir_rejected_for_file_path")
        return None

    if _search_request_needed(signal_text) and "grep_search" in available:
        return _emit_tool("grep_search", _search_args(available["grep_search"], signal_text, path_hint), request_id)

    if _search_request_needed(signal_text) and "search_in_files" in available:
        return _emit_tool("search_in_files", _search_args(available["search_in_files"], signal_text, path_hint), request_id)

    if _pwd_request_needed(signal_text) and "run_command" in available:
        return _emit_tool("run_command", _command_args(available["run_command"]), request_id)

    fallback = _first_schema_compatible_tool_call(available, path_hint, signal_text)
    _log_outgoing_tool_call(request_id, fallback)
    return fallback


def _tool_call_chunk(id: str, created: int, model: str, tool_call: Dict[str, Any], finish_reason: Union[str, None]) -> str:
    return json.dumps({
        "id": id,
...
def _chat_completion_response(chat_id: str, created_time: int, model: str, message: Dict[str, Any], finish_reason: str) -> Dict[str, Any]:
    return {
        "id": chat_id,
...
def _simple_streaming_response(chat_id: str, created_time: int, model: str, deltas: List[Dict[str, Any]], finish_reason: str) -> StreamingResponse:
    async def generator():
        for delta in deltas:
...
def _log_incoming_tools(chat_id: str, req: ChatRequest) -> None:
    if not req.tools:
        return
    tool_summaries = []
    for tool in req.tools[:20]:
    ...
    logger.info(f"[CHAT] [req_{chat_id}] early_tools={json.dumps(tool_summaries, ensure_ascii=False)} tool_choice={json.dumps(req.tool_choice, ensure_ascii=False)} parallel_tool_calls={req.parallel_tool_calls}")


def maybe_handle_openai_tool_request(req: ChatRequest, chat_id: str, created_time: int):
    _log_incoming_tools(chat_id, req)
    final_response = _final_response_from_tool_result(req)
    if final_response is not None:
        if req.stream:
            return _simple_streaming_response(
                chat_id,
                created_time,
                req.model,
    ...
    tool_call = _infer_openai_tool_call(req, chat_id)
    if not tool_call:
        return None
    if req.stream:
        return _tool_streaming_response(chat_id, created_time, req.model, tool_call)
    return _tool_call_response(chat_id, created_time, req.model, tool_call)


def _tool_streaming_response(chat_id: str, created_time: int, model: str, tool_call: Dict[str, Any]) -> StreamingResponse:
    return _simple_streaming_response(
        chat_id,
        created_time,
        model,
...
def _tool_call_response(chat_id: str, created_time: int, model: str, tool_call: Dict[str, Any]) -> Dict[str, Any]:
    return _chat_completion_response(
        chat_id,
        created_time,
        model,
...
def _last_tool_message(messages: List[Message]) -> Optional[Message]:
    for message in reversed(messages):
...
def _final_response_from_tool_result(req: ChatRequest) -> Optional[str]:
    tool_message = _last_tool_message(req.messages)
    if tool_message is None:
        return None
    tool_text = _message_text(tool_message.content).strip()
    user_text = _last_user_text(req.messages).lower()
...
def _preferred_tool_field(tool: Dict[str, Any], candidates: List[str], fallback: str) -> str:
...
def _log_outgoing_tool_call(request_id: str, tool_call: Optional[Dict[str, Any]]) -> None:
    if not tool_call:
        logger.info(f"[CHAT] [req_{request_id}] outgoing_tool_call=null")
        return
    logger.info(f"[CHAT] [req_{request_id}] outgoing_tool_call={json.dumps(tool_call, ensure_ascii=False)}")


def _arg_alias_values(value: str) -> List[str]:
    return [value, value[0].lower() + value[1:]] if value and value[0].isupper() else [value]


def _line_number_from_text(text: str) -> Optional[int]:
...
def _range_arguments(tool: Dict[str, Any], path_hint: Optional[str], text: str) -> Optional[Dict[str, Any]]:
    properties = _tool_properties(tool)
    required = _tool_required(tool)
    if not properties or not path_hint or not _looks_like_edit_request(text):
        return None

    arguments: Dict[str, Any] = {}
    path_field = _preferred_tool_field(tool, ["filePath", "path", "relativePath"], "filePath")
    arguments[path_field] = path_hint

    line_value = _line_number_from_text(text)
    if line_value is None and any(field in required for field in ["startLine", "line", "endLine"]):
        return None

    for field in ["startLine", "line", "endLine"]:
        for alias in _arg_alias_values(field):
            if alias in properties and line_value is not None:
                arguments[alias] = line_value
    ...
    if required and not all(key in arguments for key in required):
        return None
    return arguments if arguments else None


def _fallback_tool_call_from_schema(tool: Dict[str, Any], path_hint: Optional[str], user_text: str) -> Optional[Dict[str, Any]]:
    function = tool.get("function") or {}
    name = function.get("name")
    if not isinstance(name, str):
        return None

    range_args = _range_arguments(tool, path_hint, user_text)
    if range_args is not None:
        return _build_tool_call(name, range_args)
    return None


def _first_schema_compatible_tool_call(available: Dict[str, Dict[str, Any]], path_hint: Optional[str], user_text: str) -> Optional[Dict[str, Any]]:
    if not _looks_like_edit_request(user_text):
        return None
    for tool in available.values():
        fallback = _fallback_tool_call_from_schema(tool, path_hint, user_text)
        if fallback is not None:
            return fallback
    return None
...
async def chat_impl(
    ...
    if req.tools:
        ...
        logger.info(f"[CHAT] [{account_manager.config.account_id}] [req_{request_id}] tools={json.dumps(tool_summaries, ensure_ascii=False)} tool_choice={json.dumps(req.tool_choice, ensure_ascii=False)} parallel_tool_calls={req.parallel_tool_calls}")
